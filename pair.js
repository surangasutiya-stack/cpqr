...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            // ✅ OWNER NUMBER (FIXED)
            const OWNER_NUMBER = '94771657914';

            // 🔥 OWNER AUTO REACT
            if (
                config.OWNER_AUTO_REACT === 'true' &&
                senderNum === OWNER_NUMBER
            ) {
                const emoji = config.OWNER_REACT_EMOJI[
                    Math.floor(Math.random() * config.OWNER_REACT_EMOJI.length)
                ];
            ...
            if (
                config.USER_AUTO_REACT === 'true' &&
                Array.isArray(config.USER_REACT_LIST) &&
                config.USER_REACT_LIST.includes(senderNum)
            ) {
                const emoji = config.USER_REACT_EMOJI[
                    Math.floor(Math.random() * config.USER_REACT_EMOJI.length)
                ];
