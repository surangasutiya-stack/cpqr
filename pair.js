...
const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://zantamini1234:suranga20017@cluster0.jfsbncc.mongodb.net/?retryWrites=true&w=majority';
...
if (fs.existsSync(settingsPath)) {
    groupSettings = JSON.parse(fs.readFileSync(settingsPath));
}
...
case 'groupsetting': {
    ...
    const buttons = [
        { buttonId: '.linkdelete on', buttonText: { displayText: '🔗 Link Delete ON' }, type: 1 },
        { buttonId: '.linkdelete off', buttonText: { displayText: '🔗 Link Delete OFF' }, type: 1 },
        { buttonId: '.badword on', buttonText: { displayText: '🚫 Bad Word Delete ON' }, type: 1 },
        { buttonId: '.badword off', buttonText: { displayText: '🚫 Bad Word Delete OFF' }, type: 1 }
    ];

    await conn.sendMessage(
        m.chat,
        {
            image: fs.readFileSync('./media/zanta.jpg'), // BOT LOGO
            caption: '☰ *ZANTA-XMD GROUP SETTING*',
            footer: 'Tap a button to change settings',
            buttons,
            headerType: 4
        },
        { quoted: m }
...
case 'linkdelete': {
    ...
    groupSettings[m.chat] = groupSettings[m.chat] || {};

    if (args[0] === 'on') {
        groupSettings[m.chat].linkDelete = true;
        saveSettings();
        reply('✅ Link Auto Delete ENABLED');
    } else if (args[0] === 'off') {
        groupSettings[m.chat].linkDelete = false;
        saveSettings();
        reply('❌ Link Auto Delete DISABLED');
    }
}
break;
...
case 'badword': {
    ...
    if (args[0] === 'on') {
        groupSettings[m.chat].badWordDelete = true;
        saveSettings();
        reply('✅ Bad Word Auto Delete ENABLED');
    } else if (args[0] === 'off') {
        groupSettings[m.chat].badWordDelete = false;
        saveSettings();
        reply('❌ Bad Word Auto Delete DISABLED');
    }
}
break;
...
if (
    m.isGroup &&
    groupSettings[m.chat]?.linkDelete &&
    m.text &&
    /https?:\/\/|chat\.whatsapp\.com/i.test(m.text)
) {
    await conn.sendMessage(m.chat, { delete: m.key });
}

// ===== AUTO BAD WORD DELETE =====
if (
    m.isGroup &&
    groupSettings[m.chat]?.badWordDelete &&
    m.text &&
    badWords.some(word => m.text.toLowerCase().includes(word))
) {
    await conn.sendMessage(m.chat, { delete: m.key });
}
